...
class BlogController{
    static async add_data(req,res){
        ...
        try{
         ...
         // console.log(!fileExtensions.includes(imgFile.path.split('.').slice(-1)[0]))
          ...
          .catch((error)=>{
            res.statusCode=402;
            res.end(JSON.stringify({message:'Email not found',error:error})) 
          })
        ...
        //   console.log(name)
          ...
          .catch((error)=>{
            res.statusCode=402;
            res.end(JSON.stringify({message:'Blog Not added',error:error})) 
          })
          }
          else{
            return res.end(JSON.stringify({ errorValidate }));
        }
        ...
        try{
           ...
           // console.log(blog_id)
            ...
            if(result){
                ...
                res.end(JSON.stringify({message:'No blog Found.'}))
            )
        }
        catch(error){
        //console.error(error);
          ...
          .then(()=>{
            fs.rename(imgFile.path,filePath,err=>{
                if(err){
                    ...
                    res.statusCode=200;
                    res.end(JSON.stringify({message:'Blog Update successfully'})) 
                }
            })
    ...
    static async blog_pagination(req,res){
        ...
        try{
            ...
            if(result.length!==0){
                res.writeHead(200,{'Content-Type':'application/json'})
                res.end(JSON.stringify({result: result,count:count})); 
            }
            else{
              res.writeHead(404)
